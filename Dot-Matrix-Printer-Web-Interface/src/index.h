...
var state=false;var btn=document.getElementById("button");btn.onclick=function(){state=!state;if(state){btn.className="button";btn.textContent="ON";if(websocket.readyState==1){websocket.send("led_on");}}else{btn.className="button button2";btn.textContent="OFF";if(websocket.readyState==1){websocket.send("led_off");}}};const textinput=document.getElementById('inputtext');const charCountLabel=document.getElementById('charCountLabel');function sendText(){const text=textinput.value;const normalized=text.normalize("NFD");const asciiOnly=normalized.replace(/[^\x00-\x7F]/g,"");websocket.send(asciiOnly);}
textinput.addEventListener('input',function(){const charCount=textinput.value.length;charCountLabel.textContent=`Characters:${charCount}`;});fetchTemperature();setInterval(fetchTemperature,4000);window.onload=init;</script></body></html>)=====";
